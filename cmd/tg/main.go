...
func main() {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error initializing Turbo Go: %v\n", err)
		os.Exit(1)
	}
	defer app.Stop()
	...
	dispatchAction = func(actionID string) {
		...
		case "search_definition":
			...
			if !jumped {
				...
				if sym != "" {
					...
					app.SetStatusMessage("No symbol under cursor (press F12 on symbol name)")
				}
			}
		case "search_hover":
			lspClient := app.GetLSP()
			if lspClient == nil || !lspClient.IsAvailable() {
				app.SetStatusMessage("Hover requires active LSP server (gopls)")
			} else {
				ctx, cancel := context.WithTimeout(context.Background(), 2*time.Second)
		...
		case "options_lsp_status":
			...
			if lspClient != nil && lspClient.IsAvailable() {
				...
				sound.PlayError()
				if bin, found := lsp.FindGopls(); found {
					app.SetStatusMessage(fmt.Sprintf("LSP Server: gopls found at %s [Inactive/Starting]", bin))
				} else {
					app.SetStatusMessage("LSP Server: gopls not found. Using built-in AST scanner.")
				}
			}
