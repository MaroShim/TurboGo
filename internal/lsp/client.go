...
// FindGopls locates the gopls binary across system and user go bin paths.
...
// StartGoplsClient attempts to locate and launch gopls for the workspace.
func StartGoplsClient(rootDir string) (*Client, error) {
	bin, found := FindGopls()
	if !found {
		return nil, ErrServerNotFound
	}
	return StartClient("gopls", bin, rootDir)
}
...
func cleanHoverSnippet(s string) string {
	...
	// Strip markdown code fence markers (e.g. ```go ... ```)
