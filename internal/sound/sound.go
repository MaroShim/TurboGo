...
func initSoundFiles() {
	tmpDir = filepath.Join(os.TempDir(), "turbogo_sounds")
...
// It generates a band-limited tone using an IIR low-pass filter and smooth attack/decay envelope.
func generateWarmConeTone(tones []tone) []byte {
	...
	for _, t := range tones {
		...
		for i := 0; i < n; i++ {
			...
			// Apply envelope for paper cone physics (no sharp DC clicks)
			env := 1.0
			if i < attackSamples {
	...
	// Apply 1st-order IIR Low-Pass Filter to eliminate harsh metal buzz and simulate paper cone resonance
	// cutoff ~ 1400Hz
	alpha := 0.22
	filtered := 0.0
	...
	for i, x := range floatSamples {
		...
		// Map [-1.0, 1.0] to [128 - 45, 128 + 45] for pleasant, non-fatiguing volume
...
// PlaySuccess plays the classic 2-step ascending PC speaker beep (띡-빅!)
func PlaySuccess() {
	play(func() string { return successWavPath })
}

// PlayError plays the heavy low-frequency PC speaker buzz (뿌-웅!)
func PlayError() {
	play(func() string { return errorWavPath })
}

// PlayBreakpoint plays the crisp piezo click/beep on breakpoint stop (띡!)
func PlayBreakpoint() {
