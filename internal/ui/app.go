...
func (a *App) SyncDebuggerState() {
	...
	// Update user screen with live debugger output if debugging is or was active
	if a.userScreen != nil {
	...
	if a.debugger.IsActive() {
		a.statusBar.Items = []StatusItem{
			{KeyName: "F5/Alt+C", Desc: "Cont", Action: "debug_continue"},
			{KeyName: "F7/Alt+S", Desc: "Trace", Action: "debug_step_into"},
			{KeyName: "F8/Alt+N", Desc: "Step", Action: "debug_step_over"},
...
// StartDebugging compiles with debug flags and connects Delve session
func (a *App) StartDebugging() (*compiler.BuildResult, error) {
	// Auto-save
	targetFile := a.editor.FilePath
	if targetFile == "" || a.editor.Dirty {
		if targetFile == "" {
			targetFile = filepath.Join(os.TempDir(), "turbogo_dbg_main.go")
	...
	bRes := compiler.BuildDebug(targetFile)
	if !bRes.Success {
	...
	err := a.debugger.StartSession(bRes.BinaryPath, workDir, absTarget)
	if err != nil {
		return bRes, err
	}
...
func (a *App) CompileCurrent() *compiler.BuildResult {
	...
	if targetFile == "" || a.editor.Dirty {
		if targetFile == "" {
			targetFile = filepath.Join(os.TempDir(), "turbogo_temp_main.go")
			_ = a.editor.SaveAs(targetFile)
		} else {
			_ = a.editor.SaveFile()
		}
...
func (a *App) RunCurrent() (*compiler.BuildResult, *compiler.RunResult) {
	...
	workDir := filepath.Dir(a.editor.FilePath)
	rRes := compiler.RunBinary(bRes.BinaryPath, workDir, nil)
