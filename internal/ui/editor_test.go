...
func TestMenuBarComprehensive(t *testing.T) {
	...
	mb.OpenMenu(0) // File
	// File has items: New(0), Open(1), Save(2), SaveAs(3), Sep(4), Exit(5)
...
func TestEditorSaveAndLoadRoundTrip(t *testing.T) {
	tempDir := t.TempDir()
	filePath := filepath.Join(tempDir, "roundtrip_test.go")
	...
	ed1.ToggleBreakpoint(3) // BP on line 3 ("func main()")
...
func TestNavigationMultiFile(t *testing.T) {
	...
	if ed.CursorY != 1 || ed.CursorX != 5 {
		t.Errorf("expected Cursor (1, 5), got (%d, %d)", ed.CursorY, ed.CursorX)
	}
...
func TestEditorAtomicSaveAndFileSafety(t *testing.T) {
	...
	// 1. Test Atomic Save
	targetFile := filepath.Join(tempDir, "test_atomic.go")
	ed := NewEditor("", 1)
	ed.FilePath = targetFile
	ed.Lines = []string{"package main", "func main() {}"}
	ed.Dirty = true
	...
	bomFile := filepath.Join(tempDir, "bom.go")
	bomContent := append([]byte("\xef\xbb\xbf"), []byte("package main\n// with BOM")...)
	...
	if len(edBom.Lines) == 0 || edBom.Lines[0] != "package main" {
...
func TestEditorDraw_SemanticTokensOverlay(t *testing.T) {
	...
	ed := NewEditor("", 1)
	ed.Lines = []string{
		"func CalculateSum(val int) MyType {",
		"    return MyType{}",
		"}",
	}

	// Supply semantic tokens:
	// "CalculateSum" at (0, 5, 12, "function")
	// "val" at (0, 18, 3, "parameter")
	// "MyType" at (0, 28, 6, "type")
	spans := []lsp.SemanticTokenSpan{
		{Line: 0, StartCol: 5, Length: 12, TokenType: "function"},
		{Line: 0, StartCol: 18, Length: 3, TokenType: "parameter"},
		{Line: 0, StartCol: 28, Length: 6, TokenType: "type"},
	}
	...
	// Verify line 0 was retrieved and cached properly
...
func TestEditor_MouseClickAndSelection(t *testing.T) {
	...
	// 1. Click on line 1, column 5 ("func [H]ello")
	handled := ed.HandleMouseClick(interiorX, interiorY, interiorW, interiorH, interiorX+5, interiorY+1, false)
	if !handled {
	...
	if ed.CursorX != 5 {
		t.Errorf("expected CursorX 5, got %d", ed.CursorX)
	}

	// 2. Drag to line 1, column 10 ("Hello")
	handled = ed.HandleMouseClick(interiorX, interiorY, interiorW, interiorH, interiorX+10, interiorY+1, true)
	if !handled {
	...
	if selected != "Hello" {
		t.Errorf("expected selected text 'Hello', got %q", selected)
	}
...
func TestStatusBar_HandleMouse(t *testing.T) {
	...
	// 1. Click on F1 Help (x=1..8)
	act, handled := sb.HandleMouse(2, screenH-1, screenH, screenW)
	if !handled || act != "help_index" {
		t.Errorf("expected 'help_index', got handled=%v act=%q", handled, act)
	}
