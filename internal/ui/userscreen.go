...
func (u *UserScreen) Draw(screen tcell.Screen, width, height int) {
	...
	// Bottom status banner: [ Turbo Go User Screen (Alt+F5) - Press any key to return ]
	banner := " [ Turbo Go User Screen (Alt+F5) - Press any key to return to IDE ] "
	bx := (width - len(banner)) / 2
	if bx < 0 {
		bx = 0
	}
	for x := 0; x < width; x++ {
		screen.SetContent(x, height-1, ' ', nil, barStyle)
	}
